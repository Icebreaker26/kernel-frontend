...
function UmbralRow({ u, onUpdated }) {
  ...
  const guardar = async () => {
    setSaving(true);
    try {
      await apiService.put(`/control_interno/config/umbrales/${u.id}`, {
  ...
  return (
    <div className="border border-[#c084fc22] rounded-sm p-5 space-y-4 bg-[#05080f]">
      <div className="flex items-start justify-between">
        ...
        {!editing
          ? <button onClick={() => setEditing(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-[8px] tracking-widest border border-[#c084fc22] rounded-sm text-[#6aacbc] hover:text-[#a0d4e0] transition-colors">
              <Edit2 size={10} /> EDITAR
            </button>
          : <div className="flex items-center gap-2">
              <button onClick={cancelar}
                className="p-1.5 border border-[#c084fc22] rounded-sm text-[#6aacbc] hover:text-[#ef4444] transition-colors">
                <X size={12} />
              </button>
